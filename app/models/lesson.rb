...
class Lesson < ActiveRecord::Base
  ...
  scope :top_rated ,lambda { |count|
    joins('LEFT OUTER JOIN lesson_ratings ON lesson_ratings.lesson_id = less.sass.id')
    .select('less.sass.*, avg(lesson_ratings.rating) as average_rating')
    .group('less.sass.id').order('average_rating desc')
    .limit(count)
  }
